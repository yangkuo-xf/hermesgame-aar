...
import com.hermesgamesdk.skin.manager.listener.ISkinLoader;
...
public class SkinManager implements ISkinLoader {
			...
			protected Resources doInBackground(String... params) {
				try {
					if (params.length == 1) {
						...
						final String skinPkgDir = Environment.getExternalStorageDirectory() + File.separator + "hermesgameskin"
								+ File.separator + context.getPackageName() + File.separator ;

						final String skinPkgPath = skinPkgDir+ File.separator+params[0] ;
						copyAssetsSkinToLocal(skinPkgDir,params[0],context.getAssets().open( "qg_skin_pack/"+params[0]));
	...
	public ColorStateList convertToColorStateList(int resId) {
		 ...
		 */
		public static void copyAssetsSkinToLocal(String path, String fileName,InputStream input)
				throws IOException {
/*			File skinFile=new File(path);
			if (!skinFile.exists()){
				int index;
				byte[] bytes = new byte[1024];
				FileOutputStream downloadFile = new FileOutputStream(skinFile);
			...
			File skinFileDic = new File(path);
			if (!skinFileDic.exists()){
				skinFileDic.mkdirs();
			}
			File skinFile=new File(path,fileName);
			if (!skinFile.exists()){
				skinFile.createNewFile();
			}


			OutputStream outStream = new FileOutputStream(skinFile);
